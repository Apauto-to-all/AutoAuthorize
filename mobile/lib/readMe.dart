...
（3）如果你连接了校园网，并重新打开我们的程序，在未登入情况下，程序会自动帮你登入校园网，并利用通知告诉你登入情况。（不过还是需要你在连接校园网后，再运行本程序）
（4）简单地做了一个后台监测校园网的功能，如果你将程序运行在后台，会自动调用（注意：右滑退出程序是无法调用的），调用后你可以在通知栏中看到，每隔15分钟检测一次校园网是否断开，如果断开，程序会自动帮你登入校园网，并通知你。（很简陋，间隔时间最低15分钟，用处可能不大，如果程序在后台被杀，是无法使用的）
...
（4）不要关闭本程序的通知权限，推荐打开应用的横幅通知，本程序的通知只显示8秒，到期自动清除，不会打扰到你，会帮你及时了解到程序的运行情况。（除了第5条的功能4）
