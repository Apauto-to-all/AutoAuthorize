...
（3）如果你连接了校园网，并重新打开我们的程序，在未登入情况下，程序会自动帮你登入校园网，并利用通知告诉你登入情况，完成后自动退出程序。（全自动！！！不过还是需要你在连接校园网后，再运行本程序，做过一个后台监测校园网的功能，但不好用，就删除了，使用编译好后的程序，发现打开速度挺快的，即使我用的手机是性能比较渣的。所有，我感觉这个监测功能……用处不大，看你们的反馈吧，毕竟Win端已经做出了监测校园网的功能。）

6.注意：
（1）如果你需要更换学号，密码或运营商，请点击“修改账户”按钮，否则，我们会一直使用你之前通过验证时保存的账户信息。
（2）不同用户的不同网络情况，我们还没能完全覆盖到，可能会出现一些错误提示信息，请重新尝试。
（3）如果你需要使用校园网，请务必先关闭移动数据，然后打开WiFi，选择校园网进行连接，最后打开本程序进行自动登入，如果你没关闭移动数据，程序“很有可能”无法帮你登入校园网。
（4）请打开本应用的横幅通知（没找到自动设置为横幅通知的方法），本程序的通知只显示8秒，到期自动清除，状态栏也不会提示，拒绝打扰，如果不打开横幅通知，很有可能会错过一些信息。（除了第5条功能3，程序自动退出后就来不及清除了）
（5）由于获取最新版本号需要访问github，国内访问github比较慢，在检查更新时，请耐心等待，不要尝试退出“说明”界面。（之前用过github镜像网站，速度虽然快，但失效了，故不打算用）
（6）花费一周肝出来的程序，有很多地方没有完善，请见谅。（最令我吐槽的是编译成是apk文件，体积太大，有点不符合我的预期，只能这样了……）
（7）功能已经全部实现，是最初的版本，也可能是最后的版本。
''';
